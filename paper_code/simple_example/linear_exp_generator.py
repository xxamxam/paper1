...
class case_function:
    ...
    def __call__(self, b, test = False):
        ...
        if test:
            y = np.maximum(self.A @ self.x - b, 0.)
        else:
            y = cp.maximum(self.A @ self.x - b, 0.)
        result = self.c @ self.x - self.cb @ b - self.ca @ y
        return result, y
        # a = cp.Variable(b.shape)
        # constraints = [self.A @ self.x <= b + a, 0 <= a]
        # objective = cp.Maximize(self.c @ self.x - self.cb @ b - self.ca @ a,)
...
def generate_cases(m: int, n: int, k: int):
    ...
    values = []
    b_s = []
    cb_base = 3 * (np.random.rand(n)* 0.8 + 1.)
    # ca = cb * (1.05 + np.random.rand(n) * 0.1)
    A = np.random.rand(n, k) + 0.2
    for _ in range(m):
        x = (np.random.rand(k)* 1. + 0.5) * 10

        cb = cb_base *(1 + np.random.rand(n) * 0.1)
        ca = cb * (1.05 + np.random.rand(n) * 0.1)
        b_tmp = A @ x
        b = b_tmp   # пока что так, addition будет 0
